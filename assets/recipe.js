...
async function loadRecipe(recipeId) {
  try {
    const res = await fetch('data/recipes.json?v=v83');
...
// ─────────────────────────────────────────────────────────────
// ─── 계량 변환 도우미: 큰술 수치를 "큰술 + 작은술(정수)" 조합 문자열로 반환 ───
function formatTbsTsp(tbsValue) {
  let totalTsp = Math.round(tbsValue * 3);
  if (totalTsp < 1) totalTsp = 1;
...
function parseAndCalculateAmount(name, amountStr, baseS, currS) {
  ...
  if (unit.trim().startsWith('g')) {
    ...
    if (name.includes('가루') || name.includes('밀가루') || name.includes('설탕') || name.includes('소금') || name.includes('된장') || name.includes('고추장') || name.includes('버터')) {
      if (calculated >= 50) {
        ...
        appendStr = ` (약 ${formatTbsTsp(calculated / 15)})`;
      }
  ...
  if (cu === '컵' || cu === '종이컵') {
    if (calculated < 0.5) {
      return `약 ${formatTbsTsp(calculated * 16)}`;
    }
  ...
  // ── 큰술 ──────────────────────────────────────────────────────────────────
  if (cu === '큰술') {
    return formatTbsTsp(calculated);
  }
...
async function loadSeasonalIngredients() {
  try {
    const res = await fetch('data/ingredients.json?v=v83');
    if (res.ok) {
