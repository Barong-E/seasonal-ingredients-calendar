    ...
    private void callGeminiAPIForCalorie(byte[] imageBytes, PluginCall call) {
        try {
            ...
            textPart.put("text", "이 사진 속의 음식을 분석해주세요.\n" +
                    ...
                    "3. 만약 사진 속에 숟가락, 젓가락, 포크 등의 일반적인 식기류가 음식 옆에 나란히 놓여 있다면, 해당 식기류의 표준 길이를 기준자(Reference Size) 삼아 접시/그릇의 실제 지름과 높이를 3D 공간 상으로 추론하세요. 이를 바탕으로 음식의 중량(g)과 칼로리를 정교하게 가중 계산해야 합니다. 단, 식기류가 손에 들려 카메라 가까이 크게 보이는 원근 왜곡이 있는 경우는 예외로 하고 표준 1인분 기준으로 계산하세요.");
            parts.put(textPart);
