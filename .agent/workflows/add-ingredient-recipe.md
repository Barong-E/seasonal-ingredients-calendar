...
0. **데이터 관리 가이드 정독 및 식재료 선정**
   ...
   - **[1단계: 철저한 중복 검증]**: 후보를 선정하기 전에 반드시 `public/data/ingredients.json`에서 해당 식재료의 이름을 리터럴 문자열(단독 한글명, 예: "참외", "장어")로 명시적으로 검색하여 중복되지 않음을 철저히 검증하고, 이미 존재하는 유사한 식재료가 있는지 전체 목록을 직접 훑어보며 더블 체크합니다. JSON 규격 차이로 검색이 누락될 수 있는 복잡한 정규식 검색을 지양합니다.
   - **[2단계: 정확히 3개 후보 제안]**: 현재 시점(또는 요청 시점)에 맞는 제철 식재료 중 등록되지 않은 새로운 후보 **정확히 3가지**를 발굴합니다. 
   - **[3단계: 승인 대기 (조사 및 이미지 생성 절대 금지)]**: 이때는 서브 에이전트(`invoke_subagent`)를 실행하거나 DALL-E 이미지를 미리 생성해서는 **절대로 안 됩니다.** 식재료 후보와 피크타임 제철 월, 추천 이유를 정리하여 사용자에게 제안하고 허락을 기다립니다.
   - **[엄격한 제철 월 결정]**: 웹 검색을 통해 해당 식재료가 가장 맛있어지는 '맛의 절정기(피크타임)'에 해당하는 개월만 엄격하게 식별하여 제안에 포함합니다.
   - 사용자가 후보 중 추가할 대상을 명시적으로 승인('진행해', '승인' 등)을 내릴 때까지 대기합니다.

1. **상세 조사 및 리소스 생성 (승인 후 개시)**
   - 사용자가 최종 승인한 식재료에 대해서만 비로소 서브 에이전트들을 소환하여 각 식재료의 영양 정보 조사, 레시피 탐색, DALL-E 이미지 생성을 실행합니다.
   - 조사 결과와 이미지 생성 내역을 바탕으로 `implementation_plan.md`를 작성한 뒤 사용자의 명시적인 최종 결재('진행해' 등)를 득하고 2단계로 넘어갑니다.
