...
0. **데이터 관리 가이드 정독 및 식재료 선정**
   ...
   - **[엄격한 제철 월 결정]**: 단순히 1년 내내 유통되는 모든 달을 넣는 것이 아니라, **웹 검색을 통해 해당 식재료가 가장 맛있어지는 '맛의 절정기(피크타임)'에 해당하는 개월만 엄격하게 식별**합니다. (예: 후숙이 필요한 단호박은 수확기가 아닌 후숙 완료 후인 8~10월로 좁힘)
   - `DATA_GUIDE.md`의 선정 기준을 반영하여, 식재료 후보와 **검증된 피크타임 제철 월 및 추천 이유**를 함께 사용자에게 제안합니다.
...
4. **이미지 생성 및 저장**
   ...
   - **식재료 이미지 프롬프트**: `Cute 2D cartoon style vector illustration of [식재료 영문명], close up, filling the frame with minimal margins, bright colors, clean white background, simple outlines, food icon style`
     ...
     - > [!WARNING]
     - > **한국인 식문화 비주얼 엄수:** 식재료 이미지는 한국인에게 익숙한 생김새로 묘사해야 합니다. (예: 상추(`lettuce`)를 단순 영단어로 치면 서양식 동그란 양상추나 양배추로 생성되므로, 반드시 `Korean red leaf lettuce stack, individual loose fresh wavy leaves stacked` 처럼 나풀거리는 낱장 잎사귀임을 프롬프트에 상세하게 구체적으로 지정해야 합니다.)
