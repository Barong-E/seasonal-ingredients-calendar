...
> 
> **🥘 식재료-레시피 필수 연동**: 식재료 데이터를 추가하거나 업데이트할 때는 반드시 해당 식재료를 활용한 **실제 조리 과정이 있는 레시피를 최소 1개 이상** 세트로 추가해야 합니다. (식재료 업데이트 = 레시피 업데이트)
> 
> **🚫 단순 섭취 문구 기재 금지**: `popular_dish`(대표 요리) 필드에 '생과일', '그냥 먹기', '생과일로 섭취' 등 조리법이 없는 단순 섭취 방식은 기재하지 마세요. 오직 실제 요리명만 기록해야 합니다.
